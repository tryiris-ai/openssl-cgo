...
~/iris/ai.knowledge/rules/openssl-cgo/INDEX.md  (if exists)
~/iris/ai.knowledge/knowledge/global/INDEX.md
~/iris/ai.knowledge/knowledge/openssl-cgo/INDEX.md  (if exists)
```
...
Agent mistake → `/iris-rule global|openssl-cgo {rule}`
Explained something new → `/iris-learn global|openssl-cgo {context}`
